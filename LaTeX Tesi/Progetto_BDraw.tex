...
% Dico a LaTeX di usare il Computer Modern (cmr) per il greco
...
\usepackage{hyperref} % Trasforma i riferimenti incrociati, URL e l'indice in link cliccabili nel PDF e sistema dei problemi con le lettere greche
...
Il termine deriva dalle parole greche \textit{brákhistos} (\foreignlanguage{greek}{βράχιστος}, ``il più breve'') e \textit{khrónos} (\foreignlanguage{greek}{χρόνος}, ``tempo''), come coniato dal matematico svizzero Johann Bernoulli nel 1696, ma contrariamente alla visione storiografica semplificata, le radici concettuali della questione furono poste già in precedenza.
...
\begin{itemize}
    ...
    \item Il sistema deve garantire che la curva sia sia monotona strettamente crescente delle ascisse (il percorso deve avanzare sempre verso destra senza "tornare indietro")
    \item Il sistema deve garantire che la curva si trovi interamente nel semipiano $y \ge 0$ (considerando l'origine nel punto di partenza e l'asse $y$ rivolto verso il basso)
    \item Il sistema deve permettere alla coordinata $y$ di essere non monotona, consentendo alla curva di scendere e risalire per collegare punti alla stessa quota o a quote diverse
...
\begin{itemize}
    \item Il sistema deve poter generare percorsi predefiniti: linea retta, cicloide e un arco di circonferenza
    \item Il sistema deve permettere all'utente di definire un fattore di conversione (pixel per metro) tramite un'interfaccia di controllo (slider). Questo valore deve mappare le distanze visive sullo schermo a distanze fisiche reali
...
\begin{itemize}
    ...
    \item Il passaggio da una scala all'altra o il cambio di gravità deve riflettersi immediatamente sui parametri della simulazione e sul calcolo del tempo teorico senza richiedere il riavvio dell'applicazione
\end{itemize}
...
\begin{itemize}
    ...
    \item La simulazione deve preservare la correttezza delle leggi della cinematica indipendentemente dalla scala scelta. Il tempo di percorrenza ``reale'' calcolato deve essere invariante rispetto a una scala puramente visiva
\end{itemize}

\subsection{Elaborazione e Rendering}

\begin{itemize}
    \item Viene eseguito un post-processing sui punti campionati durante il disegno della curva, assicurando la monotonia strettamente crescente lungo l'asse delle ascisse
...
\begin{thebibliography}{9}
    \bibitem{buttazzo1998variations}
    G. Buttazzo, G. Dal Maso, E. De Giorgi, \textit{Calcolo delle variazioni}. Enciclopedia del Novecento, Vol. XI Suppl. II, Istituto della Enciclopedia Italiana, Roma (1998), 831--848.
    
    \bibitem{buttazzo1998one}
    G. Buttazzo, M. Giaquinta, S. Hildebrandt, \textit{One-dimensional Calculus of Variations: an Introduction}. Oxford University Press, Oxford (1998), viii+262 pp. Russian translation: Tamara Rozhkovskaya, Novosibirsk (2002).
    
    \bibitem{erlichson1998}
    H. Erlichson, \textit{Galileo's Work on Swiftest Descent from a Circle and How He Almost Proved the Circle Itself Was the Minimum Time Path}. \textit{The American Mathematical Monthly}, Vol. 105, No. 4, Apr. 1998, pp. 338--347. Published by Taylor \& Francis, Ltd. on behalf of the Mathematical Association of America. Stable URL: \url{https://www.jstor.org/stable/2589709}.
    
    \bibitem{galilei1638}
    G. Galilei, \textit{Discorsi e dimostrazioni matematiche intorno a due nuove scienze}, Giornata terza. Testo disponibile su Wikisource: \url{https://it.wikisource.org/wiki/Discorsi_e_dimostrazioni_matematiche_intorno_a_due_nuove_scienze_(Favaro)/Scienzia_nuova_altra,_de_i_movimenti_locali,_cio%C3%A8_dell%27equabile,_del_naturalmente_accelerato._Giornata_terza}.

    \bibitem{ciarlet2022}
    P. G. Ciarlet, C. Mardare, \textit{On the Brachistochrone Problem}. \textit{Communications in Mathematical Analysis and Applications}, Vol. 1, No. 1 (Feb 2022), pp. 213--240. DOI: \url{https://doi.org/10.4208/cmaa.2021-0005}.
\end{thebibliography}
