...
\usepackage[T1]{fontenc}         % Codifica dei font in uscita (migliora la sillabazione e i caratteri speciali)
\usepackage[greek,italian]{babel} % Supporto multilingua: italiano come principale, greco per i termini etimologici
\usepackage{amsmath}              % Estensioni matematiche standard della American Mathematical Society
\usepackage{amssymb}              % Simboli matematici aggiuntivi
\usepackage{geometry}             % Permette la personalizzazione dei margini e del layout della pagina
\usepackage{enumitem}             % Fornisce un controllo avanzato sugli ambienti di lista (itemize, enumerate)
\usepackage{textgreek}            % Permette di inserire lettere greche nel testo normale senza l'ambiente matematico
...
Il problema della brachistocrona, dalla parola greca composta \textit{brákhistos} (\foreignlanguage{greek}{βράχιστος}, ``il più breve'') e \textit{khrónos} (\foreignlanguage{greek}{χρόνος}, ``tempo''), rappresenta una delle pietre miliari della storia della matematica e del calcolo delle variazioni.
...
\begin{itemize}
    ...
    \item Il sistema deve permettere l'inserimento di curve tramite equazioni matematiche, oltre all'input grafico (DA IMPLEMENTARE FORSE ALLA FINE)
